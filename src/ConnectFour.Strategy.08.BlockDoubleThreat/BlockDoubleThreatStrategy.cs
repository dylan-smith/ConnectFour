...
namespace ConnectFour.Strategy.BlockDoubleThreat
{
    public class BlockDoubleThreatStrategy : IStrategy
    {
        public int MakeMove(GameState gameState, PlayerEnum whoAreYou)
        {
            ...
            if (move == -1)
            {
                ...
                if (move == -1)
                {
                    move = BlockOpponentDoubleThreatMove(gameState, safeMoves, whoAreYou);
                }
