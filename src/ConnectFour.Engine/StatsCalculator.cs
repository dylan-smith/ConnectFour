    ...
    public class StatsCalculator
    {
        ...
        private void CountValidGameStatesRecurse(GameState currentState, int nextPosition)
        {
            ...
            if (nextPosition <= 2)
            {
                ...
                s1.UpdateWithMove(x, y, PlayerEnum.PlayerOne);
                s2.UpdateWithMove(x, y, PlayerEnum.PlayerTwo);

                for (int i = y; i < 6; i++)
                {
                    s3.UpdateWithMove(x, i, PlayerEnum.Empty);
                }
            ...
            {
                currentState.UpdateWithMove(x, y, PlayerEnum.PlayerOne);
                if (IsValidState(currentState, x, y, PlayerEnum.PlayerOne))
                {
                    CountValidGameStatesRecurse(currentState, nextPosition);
                }

                currentState.UpdateWithMove(x, y, PlayerEnum.PlayerTwo);
                ...
                for (int i = y; i < 6; i++)
                {
                    currentState.UpdateWithMove(x, i, PlayerEnum.Empty);
                }
                ...
                for (int i = y; i < 6; i++)
                {
                    currentState.UpdateWithMove(x, i, PlayerEnum.GameNotDone);
                }
            }
        }
